...
#=============================================================================
# 2. Load Functions (same as original script)
#=============================================================================

# Copy all the function definitions from the original script
# (create_interaction_id, read_all_data, create_gi, find_all_interactions, 
#  add_annotations, convertToGInteractions)
...
#=============================================================================
# 3. Process the Single Resolution
#=============================================================================
...
# Save the filtered InteractionSet object for later use
saveRDS(filtered_iset, file.path(output_res_dir, "filtered_iset.rds"))

# Save a summary of the analysis
summary_stats <- data.frame(
  Resolution = resolution,
  Total_interactions = nrow(combined_iset),
  Filtered_interactions = nrow(filtered_iset),
  Samples = ncol(assay(filtered_iset))
)
write.csv(summary_stats, file.path(output_res_dir, "analysis_summary.csv"), row.names=FALSE)

cat("\nSingle resolution analysis at", resolution, "bp complete!\n")
