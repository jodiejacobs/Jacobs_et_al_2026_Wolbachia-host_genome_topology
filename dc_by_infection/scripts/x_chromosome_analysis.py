...
def analyze_x_interactions(interaction_file):
    ...
    # Count total significant interactions
...
def test_x_enrichment_vs_null(x_interactions_real, x_interactions_null):
    ...
    Tests both cis and trans separately.
    """
    from scipy.stats import fisher_exact
    
    results = {}
    
    # Test X-cis enrichment
...
def create_x_regulation_plots(has_results, compartment_results, interaction_results, 
    ...
    """
    Create visualizations for X chromosome regulation analysis.
    If null results provided, adds comparison panels.
    """
    # Determine number of rows based on whether null data is present
    ...
    # Additional plots if null data is present
    if interaction_results_null is not None and axes.shape[0] > 2:
        # Plot 7: Real vs Null - HAS proximity proportion
        ax = axes[2, 0]
        if has_enrichment_stats:
            ...
            bars = ax.bar(labels, proportions, color=colors)
            
            # Add significance stars
            if has_enrichment_stats['significant']:
                max_y = max(proportions) * 1.1
                ax.text(0.5, max_y, '***' if has_enrichment_stats['pvalue'] < 0.001 else 
                       '**' if has_enrichment_stats['pvalue'] < 0.01 else '*',
        ...
        # Plot 8: Real vs Null - X-cis proportion
        ax = axes[2, 1]
        if x_enrichment_stats and 'x_cis' in x_enrichment_stats:
            ...
            if x_enrichment_stats['x_cis']['significant']:
                ...
                ax.text(0.5, max_y, '***' if pval < 0.001 else '**' if pval < 0.01 else '*',
                       ha='center', fontsize=16)
        ...
        # Plot 9: Real vs Null - X-trans proportion
        ax = axes[2, 2]
...
def main():
    ...
    # Analyze null model if provided
    if args.null_interaction_file and args.interaction_file:
        ...
        has_proximity_results_null = analyze_has_proximity(has_sites, args.null_interaction_file)
        ...
        print(f"Real data:")
        print(f"  HAS-proximal interactions: {has_enrichment_stats['real_has_proximal']}/{has_enrichment_stats['real_total']}")
        print(f"  Proportion: {has_enrichment_stats['real_proportion']:.4f} ({has_enrichment_stats['real_proportion']*100:.2f}%)")
        print(f"\nNull model:")
        print(f"  HAS-proximal interactions: {has_enrichment_stats['null_has_proximal']}/{has_enrichment_stats['null_total']}")
        print(f"  Proportion: {has_enrichment_stats['null_proportion']:.4f} ({has_enrichment_stats['null_proportion']*100:.2f}%)")
        print(f"\nEnrichment:")
        print(f"  Fold enrichment: {has_enrichment_stats['fold_enrichment']:.2f}x")
        ...
        print("\n--- X-Cis Enrichment ---")
        print(f"Real data:")
        print(f"  X-cis interactions: {x_enrichment_stats['x_cis']['real_count']}/{x_enrichment_stats['x_cis']['real_total']}")
        print(f"  Proportion: {x_enrichment_stats['x_cis']['real_proportion']:.4f} ({x_enrichment_stats['x_cis']['real_proportion']*100:.2f}%)")
        print(f"\nNull model:")
        print(f"  X-cis interactions: {x_enrichment_stats['x_cis']['null_count']}/{x_enrichment_stats['x_cis']['null_total']}")
        print(f"  Proportion: {x_enrichment_stats['x_cis']['null_proportion']:.4f} ({x_enrichment_stats['x_cis']['null_proportion']*100:.2f}%)")
        print(f"\nEnrichment:")
        ...
        print("\n--- X-Trans Enrichment ---")
        print(f"Real data:")
        print(f"  X-trans interactions: {x_enrichment_stats['x_trans']['real_count']}/{x_enrichment_stats['x_trans']['real_total']}")
        print(f"  Proportion: {x_enrichment_stats['x_trans']['real_proportion']:.4f} ({x_enrichment_stats['x_trans']['real_proportion']*100:.2f}%)")
        print(f"\nNull model:")
        print(f"  X-trans interactions: {x_enrichment_stats['x_trans']['null_count']}/{x_enrichment_stats['x_trans']['null_total']}")
        print(f"  Proportion: {x_enrichment_stats['x_trans']['null_proportion']:.4f} ({x_enrichment_stats['x_trans']['null_proportion']*100:.2f}%)")
        print(f"\nEnrichment:")
